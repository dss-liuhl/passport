...
function Authenticator() {
  this._key = 'passport';//这个最终是session 中 的key
  this._strategies = {};
  this._serializers = []; // 是一个函数数组
  this._deserializers = []; // 是一个函数数组
...
Authenticator.prototype.init = function() {
  ...
  //将deserializeUser 绑定到这个Authenticator，
  this.use(new SessionStrategy(this.deserializeUser.bind(this)));
  //将serializeUser 绑定到这个Authenticator
 ...
 * 如果login会话已建立，这个中间件将填写req.user属性
 *
 * Note that sessions are not strictly required for Passport to operate.
 * However, as a general rule, most web applications will make use of sessions.
 * An exception to this rule would be an API server, which expects each HTTP
 * request to provide credentials in an Authorization header.
 * 例外：api 服务器 要求每一次的http请求都要求客户提供认证头和认证信息
 *
 * Examples:
 *
